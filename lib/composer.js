  ...
  var Renderer = Composer.Renderer = function(obj, options) {
    ...
    self.render = function(fn) {  
      ...
      if (self.dirty) {
        ...
        for (i; i<l; i++) {
          stackToRender.push(dirtyNodes[i]);
          /*
          var ox = dirtyNodes[i].x, oy = dirtyNodes[i].y, size = dirtyNodes[i].getSize(); 

          if (dirtyNodes[i].composer.hasOwnProperty('x') && 
              dirtyNodes[i].composer.hasOwnProperty('y') && 
              dirtyNodes[i].composer !== dirtyNodes[i]) 
          {
            ox += dirtyNodes[i].composer.x;
            oy += dirtyNodes[i].composer.y;
          }        
