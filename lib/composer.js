...
(function(window) {
  ...
  var DragManager = Composer.DragManager = function(obj) {
    ...
    self.cacheOverlap = function(data) {
      ...
      // mark all renderables that are closer as an overlap
      var di = stack.length-1;
      for (di; di>=0; di--) {
        if (stack[di] === node) { break; }
        stack[di].collides.overlappedBy.add(current);
        current.collides.overlaps.add(stack[di]);
      }      
