...
(function(window) {
  ...
  var Composer = function(canvas, options, features) {
    ...
    var self = options, version = "0.0.1", context = canvas.getContext("2d");
    ...
    self.unbind = function(name, fn) {
      if (binds[name]) {
        ...
        if (fn) {
          var i=0, l=binds[name];
          for (i; i<l; i++) {
            if (binds[name][i] === fn) {
              var start = binds[name].slice(0,i),
  ...
  var Renderer = Composer.Renderer = function(obj, options) {
    ...
    self.__defineSetter__("scale", function(v) { scale = v; });
    ...
    self.remove = function(renderable) {
      self.stack = stack = stack.filter(function(a) {
        return a!==renderable;
      });
      if (renderable.renderer === self) {
    ...
    self.clearRects = function() {
      ...
      for (i; i<l; i++) {
        ...
        obj.composer.context.fillRect(Math.floor(r.x*scale),Math.floor(r.y*scale),Math.ceil(r.width*scale),Math.ceil(r.height*scale));
      }
      clearRects = [];
    };
    ...
    self.render = function(fn) {
      ...
      if (self.dirty) {
        ...
        for (i; i<l; i++) {
          ...
          {
            ox += stack[i].composer.x*scale;
            oy += stack[i].composer.y*scale;
          }

          if ((ox >= 0 || ox + size.width >= 0) && (size.width + ox <= obj.canvas.width/scale || ox <= obj.canvas.width/scale) &&
              (oy >= 0 || oy + size.height >= 0) && (size.height + oy <= obj.canvas.height/scale || oy <= obj.canvas.height/scale))
          {
        ...
        if (self.dirty) {
          obj.composer.context.fillStyle = backgroundColor;
          obj.composer.context.fillRect(0,0, obj.composer.canvas.width, obj.composer.canvas.height);
        }
        self.dirty = false;
        needsRender = true;
        // Partial rendering
      ...
      if (needsRender && stackToRender.length > 0) {
        ...
        obj.composer.context.save();
        obj.composer.context.scale(scale, scale);
        var ri=0, rl=stackToRender.length;
        for (ri; ri<rl; ri++) {
          stackToRender[ri].dirty = false;
          stackToRender[ri].render(data);
        }
        obj.composer.context.restore();
      }
    };
  ...
  var Renderable = Composer.Renderable = function(obj, parts) {
    ...
    obj.__defineSetter__("x", function(value) {
      if (value !==coords.x) {
        ...
        if (obj.composer && obj.composer.renderer) {//} && !obj.composer.renderer.dirty) {
          obj.dirty = true;
          obj.composer.renderer.sort(false,true);
    ...
    obj.__defineSetter__("z", function(value) {
      if (value !== coords.z) {
        ...
        if (obj.composer && obj.composer.renderer) { //} && !obj.composer.renderer.dirty) {
          ...
          obj.composer.renderer.remove(obj);
  ...
  var DragManager = Composer.DragManager = function(obj) {
    ...
    self.cacheOverlap = function(data) {
      ...
      var oi=0, ol=current.collides.overlappedBy.length();
      for (oi=ol-1; oi>=0; oi--) {
        var overlapper = current.collides.overlappedBy.item(oi);
        if (overlapper) {
      ...
      var stack = current.composer.renderer.stack;
      var i=0, l = stack.length;
      var dx, dy, dsize = current.getSize(), ox = 0, oy = 0;
      ...
      for (i; i<l; i++) {
        ...
        if (stack[i].collides &&
            (
              current.collides.aabb(dx            , dy)              || // Upper left
              current.collides.aabb(dx            , dy+dsize.height) || // Top right
              current.collides.aabb(dx+dsize.width, dy+dsize.height) || // Bottom right
              current.collides.aabb(dx+dsize.width, dy)                 // Upper right
            )
          )
        {
          // Mark nodes that node is overlapping dirty
          if (stack[i] !== current && current.composer !== current && stack[i].composer !== stack[i]) {
    ...
    document.addEventListener("mousemove", function(ev) {
      var scale = obj.composer.renderer.scale;
      ...
      if (node) {
        ...
        // Mark nodes that node is overlapping dirty
        var stack = obj.composer.renderer.stack, i=0,
            l = stack.length, dx = node.x, dy = node.y,
            dsize = node.getSize();
        ...
        ev.canvasOffset = {x:mx/scale, y:my/scale};
        node.dragging(ev);
    ...
    document.addEventListener("mousedown", function(ev) {
      ...
      var ox = mx/scale, oy = my/scale,
          i=0, stack=obj.composer.renderer.stack, // TODO: use a pre-computed "visible" render list
          l=stack.length-1;
  ...
  var Droppable = Composer.Droppable = function(obj) {
  ...
  var DropTarget = Composer.DropTarget = function(obj) {
    obj.test = function(droppable) {
      // test whether the incomming droppable is allowed to drop here
    };
  };

  var DropManager = Composer.DropManager = function(obj) {
    var self = {}, targets = [];
