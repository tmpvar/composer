  ...
  var Renderer = Composer.Renderer = function(obj, options) {
    ...
    self.render = function(fn) {  
      ...
      if (self.dirty || firstRender) {
        ...
        stackToRender = dirtyNodes;
        var di=0; dlen=dirtyNodes.length, i=0; l=stack.length;
        
        // this is done for 2 reasons.
        // 1) loop through the bigger array once
        // 2) items added to the render stack will be in the proper rendering
        for (i; i<l; i++) {
          var ox = stack[i].x, oy = stack[i].y, osize = stack[i].getSize();

          for (di=0; di<dirtyNodes.length; di++) {
            var dx = dirtyNodes[di].x, dy = dirtyNodes[di].y, dsize = dirtyNodes[di].getSize();
            
            var found = false;
            if (// Upper left
                (dx >= ox && dx < ox + osize.width && dy > oy && dy < oy + osize.height) ||
                // Lower Right
                (dx + dsize.width  > ox && dx + dsize.width  < ox + osize.width && 
                       dy + dsize.height > oy && dy + dsize.height < oy + osize.height)  ||
                // Lower Left
                (dx  > ox && dx  < ox + osize.width && 
                       dy + dsize.height > oy && dy + dsize.height < oy + osize.height)  ||
                // Upper Right
                (dx + dsize.width  > ox && dx + dsize.width  < ox + osize.width && 
                       dy > oy && dy < oy + osize.height)
              )
            {
              stackToRender.push(stack[i]);
            }
