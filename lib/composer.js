...
(function(window) {
  ...
  var Draggable = Composer.Draggable = function(obj) {
    ...
    obj.dragstart = function(ev) {
      obj.setPos(obj.getPos().x, obj.getPos().y, draggingZ);
      mouseOffset = {x:ev.canvasOffset.x-obj.getPos().x, y:ev.canvasOffset.y-obj.getPos().y};
    };

    obj.dragging = function(ev) {
      obj.setPos(ev.canvasOffset.x-mouseOffset.x, ev.canvasOffset.y-mouseOffset.y, draggingZ);
    };

    obj.dragend = function(ev) {
      mouseOffset = {x:0, y:0};
      obj.setPos(obj.getPos().x, obj.getPos().y, obj.getPos().z-1);
    };
