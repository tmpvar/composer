...
(function(window) {
  ...
  var Collidable = Composer.Collidable = function(obj) {
    ...
    self.overlappedBy = {
      ...
      dirty : function() {
        ...
        for (i; i<l; i++) {
          ...
          overlappedBy[i].collides.overlappedBy.dirty();
        }
      }
    };
    ...
    self.aabb = function(x,y) {
      ...
      if (composer.x && //hasOwnProperty('x') && 
          composer.y && //hasOwnProperty('y') && 
          composer  !== obj) 
      {
        ox = composer.x;
        oy = composer.y;
      ...
      if (node) {
        ...
        for (i; i<l; i++) {
            ...
            )
          {
            current.dirty = true;
            if (current !== node) {
              current.collides.overlappedBy.dirty();
            }
          }
        }
