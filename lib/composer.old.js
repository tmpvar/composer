...
(function (window) {
  ...
  Renderer = Composer.Renderer = function (obj, options) {
    ...
    // TODO: custom implementation or remove completely.
    ...
    self.insertAt = function (renderable, pos) {
      var start = stack.slice(0,pos), 
          rest = stack.slice(pos);
      stack = start;
      stack.push(renderable);
      
      if (rest && rest.length) {
        stack.push.apply(stack, rest);
      }
      renderable.renderer = self;
      return self;
    };
  ...
  Renderable = Composer.Renderable = function (obj, parts) {
    ...
    obj.__defineSetter__("z", function (value) {
      if (value !== coords.z) {
        coords.z = value;
        if (obj.composer !== obj && obj.composer && obj.composer.renderer) {
          obj.dirty = true;
          obj.composer.renderer.remove(obj);
          // TODO: sort of actually calculate the new position.
